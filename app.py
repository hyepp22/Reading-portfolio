...
# 구글 시트 데이터 읽기 헬퍼 함수 (CSV 웹 요청 방식 - 라이브러리 충돌 없음)
        ...
        with st.form("student_login_form"):
            ...
            if submit_student:
                ...
                if not df_students.empty:
                    ...
                    st.error("구글 시트에 접근할 수 없습니다. Secrets 설정 및 시트 웹 공유 상태를 확인하세요.")
    ...
    with st.form("reading_log_form", clear_on_submit=True):
        ...
        if submitted:
            if not (book_title and author and summary and quote and q_na and thought):
                ...
                st.balloons()
                st.success("독서 기록이 저장되었습니다! (스프레드시트에 연동됨)")
...
# ---------------------------------------------------------
    ...
    if df_students.empty:
        st.error("구글 시트 'students' 탭을 불러올 수 없습니다.")
        st.stop()
